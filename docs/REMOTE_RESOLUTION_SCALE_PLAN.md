...
- 已实现 RDP `Auto` 加 18 个稳定 ID 的常用分辨率预设，保留发布版本的四挡偏好迁移。
...
本文件保留 RDP、RustDesk 分辨率选择、本地缩放、双指画布交互和 RustDesk Android portrait 的审查依据与实施边界。v2.4 已完成控制端实现和自动化验证；本节后续的计划条目仍定义真机验收、外部 Android 被控端实现与发布条件，不能把自动化通过等同于端到端设备验收。
...
| RustDesk 官方手机版双指基线 | 内置 vendor 的 Flutter 代码已实现 `ScaleGestureRecognizer` 状态机；移动端分支更新本地 `CanvasModel`，手势说明明确为 `Canvas Zoom` | **官方行为证据已确认；本项目尚未接入或加开关** | 对齐 `scale + focalPointDelta`、焦点锚定、重置和单独开关；项目额外实现 pan 边界 clamp 与 native/ArkTS 唯一事件所有者 |
...
| RustDesk 流统计快照 | 当前工作树已增加 Rust 每连接快照、C++ Bridge `getDiagnostics()`、NAPI `getRustDeskDiagnostics`/`getLocalResourceStats`、ExtensionLoader/type、RemoteDesktop 1 秒轮询和 `RustDeskDiagnosticsHud`；设置只完成本地持久化字段，尚无可见诊断开关 Row | **诊断观测链路已贯通到会话 HUD 的部分实现；不改变远端分辨率、画布缩放或 portrait 几何能力** | 补设置 Row/Pro 依赖、session generation/生命周期、字段来源和 ABI/定向/真机测试；实际分辨率必须改为逐帧 geometry |
...
- RustDesk 协议在 `rustdesk_vendor/libs/hbb_common/protos/message.proto:155–184` 已定义 `TouchScaleUpdate` 和 pan 事件；上游 Flutter 在 `rustdesk_vendor/flutter/lib/common/widgets/remote_input.dart:460–500` 对桌面控制端发送远端 scale、对移动控制端使用本地 canvas scale，说明两种语义应保持分离。
- RustDesk 官方手机版手势基线位于 `rustdesk_vendor/flutter/lib/common/widgets/gestures.dart:5–193`：`CustomTouchGestureRecognizer` 继承 `ScaleGestureRecognizer`，显式区分 one/two/three-finger 状态；双指回调直接保留 `ScaleUpdateDetails.scale`、`focalPoint` 和 `focalPointDelta`。
- 官方移动控制端分支位于 `rustdesk_vendor/flutter/lib/common/widgets/remote_input.dart:448–510`：先清除待定 tap；普通移动端调用 `canvasModel.updateScale(d.scale / _scale, d.focalPoint)`，随后按 `d.focalPointDelta` 调用 `panX/panY`；手势结束只重置本次手势的 `_scale`，不把结果写回设置百分比。官方桌面控制端分支才把增量编码为 `scale * 1000` 发送远端 scale，并以 `0` 结束。
- 官方画布变换位于 `rustdesk_vendor/flutter/lib/models/model.dart:1919–1935、2592–2637`：缩放上下限由图片尺寸与 viewport 动态计算，`updateScale()` 用 focal point 公式保持手指下内容稳定；但 `panX/panY` 在该路径只是直接累加，没有显式内容边界 clamp，因此本项目应在对齐行为的基础上补足四边边界约束。
- 官方渲染和重置位于 `rustdesk_vendor/flutter/lib/mobile/pages/remote_page.dart:1141–1161`、`rustdesk_vendor/flutter/lib/utils/image.dart:93–127`、`rustdesk_vendor/flutter/lib/common/widgets/toolbar.dart:430–434`：画布变换进入 ImagePainter 的 scale/x/y，移动端提供 `Reset canvas`；`rustdesk_vendor/flutter/lib/mobile/widgets/gesture_help.dart:304–345` 将两指移动标为 `Canvas Move`、捏合标为 `Pinch to Zoom / Canvas Zoom`。这些是本项目手机版体验的对齐基线，不是远端应用缩放能力证明。
...
- 2026-07-20 当前工作树的 RustDesk 代码已在 `rustdesk_ffi/src/lib.rs:269–305` 增加 `RustDeskStreamStats`，在 `:382` 挂到每个 `RustDeskClient`，在 `:890–897` 按初始 `peer_display_size()` 创建，并由 `connector.rs:1017–1027`、`:1084–1099` 更新视频/音频/TestDelay/cadence 统计；`lib.rs:1033–1048` 导出非破坏性 `rustdesk_get_stream_stats()`。
- 当前 C++ 变动在 `entry/src/main/cpp/rustdesk/rustdesk_bridge.cpp:47–65、:726–761` 声明并读取 Rust C ABI，按 callback 计数器和 FFI 快照组装 `RustDeskDiagnosticsStats`；`extension_loader_napi.cpp:561–725、:2731–2736` 已增加 NAPI 诊断/本机资源查询，`ExtensionLoader.ets`/`rdpnapi.d.ts` 已有 wrapper/type。`RustDeskDiagnosticsStats.width/height` 仍来自 callback 的 `FfiVideoFrame` 尺寸或初始 FFI fallback，必须继续核验它们是不是逐帧实际尺寸。
- 当前 `HostListPage.ets` 只新增了 `rustdeskShowDiagnostics`/`rustdeskShowProDiagnostics` 的本地 Preferences/AppStorage 读写和保存 helper；当前 diff 尚未发现对应设置 Row。`RemoteDesktop.ets` 已有 1 秒轮询、连接/后台/断开清理和 `RustDeskDiagnosticsHud` 接入，但仍需严格 session generation、Pro 采样门控和安全区/设备布局验收。诊断设置“存储已部分实现、会话显示已部分接通、入口未完成”。

### 1.3 2026-07-20 代码变动同步结论

当前分支 `codex/rustdesk-performance-diagnostics` 位于 `1f16f361a`，工作树有 Rust FFI、C++ Bridge/NAPI/decoder、ArkTS wrapper/type 和本地设置持久化的未提交变动。根据静态审查，新增链路的准确状态如下：

| 层 | 已发生的变动 | 仍缺少的能力 | 对本计划的影响 |
|---|---|---|---|
| Rust FFI 数据结构 | `#[repr(C)] RustDeskStreamStats`，包含 version/state、TestDelay、目标码率、视频消息/帧/关键帧、编码字节、音频帧、cadence gap、实际 codec、初始宽高和连接路径 | `updated_at`、实际逐帧 geometry、displayIndex、rotation、stride、geometryEpoch、有效性/会话代号 | 可以支持诊断观测；不能替代 `RemoteVideoGeometry` |
| Rust streaming 线程 | 用 `Arc<Mutex<RustDeskStreamStats>>` 按连接保存，并更新视频、音频、TestDelay、cadence 数据；断流时将 state 置为 disconnected | 更新策略还没有与 UI session/generation 对齐；码率/帧率滚动窗口仍由 native 负责 | 阶段 0/6 可使用这些计数做基线，但需补 ABI/并发/重连测试 |
| Rust C ABI | 已导出 `rustdesk_get_stream_stats(handle, out_stats) -> bool`，以 copy snapshot 返回；C++ Bridge 已读取并转换部分字段 | 缺 `static_assert`/version 校验、完整错误状态、IPC/非 real-core 降级和字段来源标记 | 诊断阶段 2 的 Rust/C++ 主体已部分完成，阶段门尚未满足 |
| C++ Bridge | 已声明并实现 `RustDeskBridge::getDiagnostics()`，合并 callback 帧计数和 FFI TestDelay/码率/cadence 等字段 | 缺 `static_assert`/ABI 版本校验、IPC/非 real-core 语义、完整错误状态和真实逐帧 geometry 证明 | 诊断桥已部分可读，但仍不能对外宣称完整诊断可用 |
| NAPI/ArkTS 接口 | 已增加 `getRustDeskDiagnostics`、`getLocalResourceStats`、`DecoderTelemetrySnapshot`，ExtensionLoader/type、RemoteDesktop 1 秒 poll 和 `RustDeskDiagnosticsHud` 已接入 | 缺显示设置 UI 行、session generation/lifecycle 严格校验、Pro include/采样门控、safe-area/Phone/Pad/PC 布局和测试 | 进入诊断阶段 3/4/5 的部分实现；只增加观测，不改变原有分辨率/缩放结论 |
| 分辨率/缩放/竖屏 | 本次没有新增 resize、pinch、CanvasTransform、rotation、stride 或显示器控制代码 | 原计划中的所有功能链路仍需按 v2.3 方案实施 | 仅增加可观测性，不改变原有审查结论 |

因此，后续实施顺序应先把现有统计快照收敛为安全的 per-session 诊断接口，再将它作为 Android portrait、4K、缩放和分辨率切换验收的观测工具；不得把这批代码误记为分辨率或竖屏修复完成。
...
│  ├─ 双指操作：`canvasGestureZoomEnabled` 开关、默认目标、溢出后的拖动/滚动说明、重置策略
...
└─ RustDesk（协议区）
   ...
   ├─ 连接诊断：基础诊断与专业性能数据（本地-only，Pro 为基础诊断子项）
...
| 双指画布缩放开关与默认目标 | 显示与交互 / 双指操作；策略键 `canvasGestureZoomEnabled` | 客户端设备默认；Phone/Pad 建议默认开启以对齐官方手机版，PC/无触摸设备建议默认关闭或不展示 | 开启时按官方 `scale + focalPointDelta` 语义进入画布目标；关闭时恢复既有双指右键/滚轮或显式触控板语义；会话可临时覆盖 |
...
| RustDesk 连接诊断 | RustDesk / 连接诊断 | `rustdeskShowDiagnostics`、`rustdeskShowProDiagnostics` 仅保存在本机 Preferences/AppStorage，不进入 CloudStore/主机 JSON；Pro 依赖基础诊断 | 会话中按 session 查询；当前代码已有存储、native/NAPI 查询、poll 和 HUD，仍需 UI Row、依赖置灰、严格生命周期和测试 |
...
- `canvasGestureZoomEnabled` 是用户可见的画布手势开关，不是远端应用缩放开关；远端应用缩放仍由 RustDesk 区的独立安全开关和当前 Peer capability/权限状态共同决定。关闭画布开关时，不得静默吞掉双指，必须显示或恢复当前协议既有的双指控制语义。
...
#### 5.5.1 RustDesk 官方手机版行为对齐（本项目的本地画布目标）

内置 RustDesk Flutter 代码可以作为 Phone/Pad 连接画面的交互基线，但不能被误读为远端应用缩放实现：

1. 官方 `CustomTouchGestureRecognizer` 继承 `ScaleGestureRecognizer`，把 one-finger pan、two-finger scale+pan、three-finger vertical drag 分成独立状态；双指回调使用累计的 `ScaleUpdateDetails.scale`、两指中心 `focalPoint` 和中心位移 `focalPointDelta`。
2. 官方移动端在每次更新时以 `relativeScale = details.scale / lastGestureScale` 更新本地 canvas，以 `focalPoint` 做焦点锚定，再按 `focalPointDelta` 更新 pan；手势结束只把本次手势的 `lastGestureScale` 复位为 1，不把临时结果写回设置中的百分比缩放。
3. 官方 `CanvasModel.updateScale()` 按图片和 viewport 计算动态 min/max scale，并用焦点公式保持手指下内容稳定；`ImagePainter` 使用同一组 x/y/scale 绘制画面；移动端菜单提供 `Reset canvas`。官方 `panX/panY` 仍是直接累加，本项目要额外增加内容边界 clamp、窗口变化后的重新 clamp 和“内容未溢出时不产生无意义平移”。
4. 官方移动端这条路径不发送远端 scale 包；官方桌面端分支才把增量编码为 `scale * 1000` 并发送远端事件。因此 Phone/Pad 的“官方手机版对齐”必须实现为本地 `CanvasTransform`，不能复用 RustDesk Android 远端应用 touch-scale 的开关或状态。

计划中的实现合同：

```text
canvasGestureZoomEnabled = false
  → 保留当前协议的双指右键/滚轮/显式触控板语义

canvasGestureZoomEnabled = true
  → TwoFingerCandidate
  → relativeScale = recognizerScale / lastGestureScale
  → CanvasTransform.updateScale(relativeScale, focalPoint)
  → CanvasTransform.pan(focalPointDelta)
  → clamp(scale, panX, panY)
  → end/cancel 时清理手势状态，不发远端点击或远端 touch-scale
```

开关应作为 `显示与交互 → 双指操作` 的持久化设备默认，并允许会话内临时覆盖。Phone/Pad 建议默认开启以对齐官方手机版；PC 或无触摸设备建议默认关闭/不展示。无论默认值如何，设置页和会话菜单都必须显示当前状态、关闭后的回退语义和“重置画布”入口。远端应用目标继续由 RustDesk 独立 capability/权限开关控制，不能因为画布开关开启就自动切换目标。
...
7. 记录当前 RustDesk 流统计快照代码的 ABI、初始宽高来源和重连行为；明确它只能作为诊断观测层，不能替代逐帧 geometry。
8. 固定内置 RustDesk vendor 的官方手机版手势基线：`CustomTouchGestureRecognizer` 状态、`scale/focalPoint/focalPointDelta` 语义、CanvasModel 焦点公式、动态缩放上下限、Reset canvas 和官方的移动端/桌面端分支差异。
9. 增加设计阶段使用的 feature flag 方案：RustDesk 远端分辨率默认关闭；本地新缩放失败时可强制 Fit；`canvas_gesture_zoom_enabled`（对应用户设置键 `canvasGestureZoomEnabled`）、远端 touch-scale、Android geometry v2、设置布局 v2 和诊断 HUD 单独灰度。

**退出条件**：四档 RDP 的实际输入值可追踪；RustDesk 的目标 Peer 版本、显示器和权限样本齐全；设置项的协议归属、保存范围和 Phone/Pad/PC 布局已冻结；诊断快照的 ABI、session 隔离和初始宽高局限已记录；当前行为已有可回退基线。
...
6. 按官方手机版算法实现 `Idle → TwoFingerCandidate → CanvasTransform/Cancel`：以累计 recognizer scale 的相邻比值更新 zoom，以 `focalPoint` 保持焦点，以 `focalPointDelta` 更新 pan；缩放比例动态限幅，并对 pan 增加官方路径没有提供的内容边界 clamp。
7. 确定 XComponent/native 与 ArkTS overlay 的唯一 gesture owner，阻止重复转发；三指面板保留，现有双指右键/滚轮迁移到显式触控板模式或独立入口。
8. 在 `HostListPage.ets` 增加共享“显示与交互”设置区：`canvasGestureZoomEnabled` 开关、默认目标、溢出滚动说明、重置策略和 Auto 方向；Phone/Pad 默认开启建议，PC/无触摸设备默认关闭/不展示；在 `RemoteDesktop.ets` 会话菜单增加临时覆盖与“恢复设置默认值”。
9. 完成开关关闭时的兼容回退、触摸平移与点击冲突处理；开关开启的本地画布路径不得发送远端 scale/点击/滚轮，触控板和鼠标继续沿用相对/绝对语义但统一进行边界校验。

**退出条件**：画面模式不再只有 Toast；所有固定缩放档都可观察到实际变化；Phone/Pad 开启 `canvasGestureZoomEnabled` 后，双指行为与官方手机版的 canvas zoom/pan 语义一致且能查看溢出内容；关闭后旧双指语义可用；设置默认和会话临时覆盖语义一致；坐标 round-trip、焦点锚定、缩放/平移边界、黑边、裁剪、平移和窗口变化测试通过；出现异常时可一键回退 Fit。
...
4. 分别在 `canvasGestureZoomEnabled` 开/关下进行双指画布焦点缩放、四边 pan、三指控制面板、旧双指右键/滚轮回退模式的冲突和取消测试；开启时确认本地画布路径没有远端 scale/点击/滚轮副作用。
...
8. 收集首帧时间、帧率、端到端输入延迟、CPU/GPU、内存峰值、切换耗时和黑屏/花屏次数；若启用当前流统计快照，记录其版本、state、实际 codec、cadence gap 和连接路径，但将 width/height 标注为初始显示尺寸，直到逐帧 geometry 接通。

**退出条件**：发布门禁、设备矩阵、性能基线、回滚演练和用户文案审查全部完成，且没有未解释的分辨率/缩放状态不一致；如使用 RustDesk 流统计，必须同时保留初始 PeerInfo 尺寸与逐帧实际尺寸的来源说明。
...
- 画布手势：对照 RustDesk 官方手机版的累计 scale、focal point 锚定、focalPointDelta pan、手势结束复位和 Reset canvas；同时覆盖缩放上下限、项目额外的 pan clamp、溢出内容四边、`TwoFingerCandidate` 取消、开关回退、双指与三指面板/右键/滚轮仲裁、native/ArkTS 重复事件防护。
...
- 诊断观测：`RustDeskStreamStats` 默认值/ABI/并发 snapshot、Bridge 字段来源、NAPI session 查询、decoder active 快照、本机 CPU/RSS unavailable 语义和关闭开关后的零轮询；诊断 width/height 必须能区分初始 PeerInfo 与逐帧 `FfiVideoFrame`。

### 7.2 Native/Rust/构建验证

- 复用并扩展 RustDesk FFI host tests，覆盖 `PeerInfo`、版本分支、`change_display_resolution` oneof、错误和取消；对当前 `RustDeskStreamStats` 增加默认值、TestDelay/codec/cadence 更新、空 handle、并发 snapshot、断开 state 和布局尺寸测试。
- 增加 C++ Bridge/NAPI 诊断测试：C ABI size/alignment/version、空指针/锁失败、FFI/IPC/非 real-core 降级、session teardown/generation、decoder active handle、NAPI 字段来源和本机资源首次采样语义。
...
| 客户端设备 | HarmonyOS PC、Phone、Pad；窗口调整、全屏、分屏、旋转、不同 DPI；Phone/Pad 开关默认开启与手动关闭回退 |
...
- **诊断尺寸被误读**：当前 `RustDeskStreamStats.width/height` 在连接时从 `peer_display_size()` 初始化，不能证明每个编码/解码帧的真实尺寸；诊断 UI 必须明确字段来源，不能把它当作 portrait 修复证据。
...
- `canvas_gesture_zoom_enabled`：双指画布 zoom/pan；对应设置策略键 `canvasGestureZoomEnabled`，关闭时恢复旧双指输入语义；Phone/Pad 的建议默认值为开启，PC/无触摸设备为关闭或不展示。
...
- `rustdesk_stream_diagnostics`：RustDesk per-connection stream snapshot、native/NAPI 诊断和 HUD；关闭时不启动轮询/资源采样，保留协议连接行为。
...
6. 双指默认目标是否确定为“画布”；建议默认画布，Phone/Pad 的 `canvasGestureZoomEnabled` 默认开启以对齐官方手机版，远端应用目标通过控制条显式切换，避免一次手势同时改变本地和远端状态。
...
- 双指画布缩放能够在远端画布溢出时按官方手机版语义保持焦点、平移到边界且不误发远端输入；`canvasGestureZoomEnabled` 开关关闭后旧双指语义可回退；设置缩放和手势缩放状态独立；
...
- 如交付诊断观测层，其统计按 session/generation 隔离，C ABI 布局经过双 ABI 校验，且 UI 明确区分初始化 PeerInfo 尺寸与逐帧实际尺寸；诊断数据不能在日志读取时被清零；
...
当前阶段的完成状态为：**控制端的 RDP 预设、本地显示缩放、双指画布缩放/平移、RustDesk 分辨率控制消息、显示快照和 Android 旋转几何更新、设置入口、Rust/C++/ArkTS 桥接与自动化门禁均已完成。真实 Windows/RustDesk/Android/Phone/Pad 设备矩阵仍是发布前必做项；外部 RustDesk Android 被控端尚未完成 touch-scale 注入，因此“远端应用双指缩放”只对实现该官方协议的 peer 生效，不能以客户端发送成功替代 Android 应用实测。**
