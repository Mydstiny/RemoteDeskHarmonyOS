...
- VNC 全屏 ZRLE 现场定位已完成：有线读取通常仅 3–7ms，但 2940×1848/1912 全屏更新在设备端仍耗时约 0.79–0.81s（inflate 约 0.31s、tile 解码约 0.39s、写回约 0.09s）。当前协议热路径优化已通过 native 178/178 与两项 Hvigor 门禁并安装签名 HAP；最新安装后尚未补到新的会话样本。继续任务时优先做同一设备的 raw/自适应编码或解码调度 A/B，保持画质与增量协议边界。
