...
3. Reproduce intermittent RDP `0x10` with Application state, RDP connection and routing/gateway diagnostics selected and matching HDC hilog. Preserve the exact native ErrorInfo, transport-end reason, network generation/availability, reconnect timeline and whether the displayed code came from the raw native message or the ArkTS fallback; distinguish genuine `E-RDP-ERRINFO-0x00000010` (remote Windows DWM crash) from a generic failure mislabeled `0x10`.
...
1. Triage any consolidated device findings against the committed item boundary; use the schema-v4 generation/matrix chain for flip issues, and remove the RDP generic `0x10` ambiguity after the raw incident source is captured. Preserve the exact protocol, device type, window mode, decoder and reproduction sequence.
