...
- Same-page SSH host switching now rejects stale async connect/attach/data/PiP
  ...
  its first frame.
  The fallback FIFO is host-scoped, returns unconfirmed batches across lifecycle handoffs, and the visible layer stays masked until xterm-ready; ACK timeout reloads are generation-guarded.
  `default@OhosTestCompileArkTS` passed; targeted physical-touchpad Rust tests are
  `3 passed, 0 failed`; `assembleHap` is blocked by the existing
  `ssh_adapter.cpp:2246` mixed-worktree error. Device installation/acceptance
...
- The current RDP increment passes `default@OhosTestCompileArkTS`,
  ...
  error in `ssh_adapter.cpp:2246`; the OHOS callback binary requires a target
...
## Next
- WP-T4 canvas user acceptance and benchmark using shared terminal fixtures.
- Finish independent review of the current same-page SSH binding-generation and commit,
  ...
  reused-renderer rebind/reload, per-host output FIFO and ready-gated
  presentation.
