...
> 建立日期：2026-08-19
> 当前工作树：`codex/moonlight-complete-upgrade` / 代码检查点 `ec4c5573`（M0–M9 SSH 增量保留；API23-targeted HAP 已在 API24 Phone/2in1 覆盖既有连接、鉴权、窗口、后台通知和 SFTP 设备矩阵；M6 日志和 M8 终端输出/replay 已按真实 UTF-8 字节有界，xterm ACK 失败回灌也重新执行 8MiB/512 块硬 admission；ArkTS 待发送输入同时受实际字节与 512 个 item 硬上限保护，256KiB paste 以 Unicode-safe batch 原子接纳，WebMessagePort 双端单帧同时受 64KiB UTF-8 字节上限保护，异步复制结果按 lifecycle/surface/request 拒绝旧文档回调，native renderer 输入载体的模块级状态在 host 清理时释放且可惰性复用；会话日志、广播输入及生产力加密 Store 继续 fail closed；API26 工具链和最终物理设备矩阵仍待外部条件）
