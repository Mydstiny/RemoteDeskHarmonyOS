...
> 建立日期：2026-08-19
> 当前工作树：`codex/moonlight-complete-upgrade` / 代码检查点 `f33be9d1`（M0–M9 SSH 增量保留；API23-targeted HAP 已在 API24 Phone/2in1 覆盖既有连接、鉴权、窗口、后台通知和 SFTP 设备矩阵；M6 日志和 M8 终端输出/replay 已按真实 UTF-8 字节有界，ArkTS 待发送输入及 256KiB paste 上限现也与 native UTF-8 字节语义一致，paste 分块不拆 surrogate pair 且在页面队列原子接纳；会话日志、广播输入、WebMessagePort 及生产力加密 Store 继续 fail closed；API26 工具链和最终物理设备矩阵仍待外部条件）
