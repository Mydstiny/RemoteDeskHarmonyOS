...
## VNC 渲染器启动失败回归调查（2026-08-02，仅调查与计划增补）

- 用户现场：VNC 当前无法正常连接，UI 报渲染器初始化失败。
- 静态高置信根因：当前 `RemoteDesktop.doConnect()` 先调用 `initRenderer()`，后调用
  `loader.connect()`；未提交的 native owner 门禁在 `NapiInitRenderer()` 进入
  `GLRenderer::Init()` 前要求 `SharedSessionSinkOwnerLease` 已有 active owner。
  VNC 的 `SessionContext`/owner 只在后续 `NapiConnect()` 创建和激活，因此冷启动稳定返回
  `rc=-1`，失败点早于 EGL/GL。不能通过放宽 owner 校验、复用旧 owner 或伪造 token 绕过。
- 计划已合并增补：
  `docs/superpowers/plans/2026-08-01-vnc-certificate-settings-relay-complete-repair-plan-v3.md`。
  增补要求 reservation → renderer owner bind → activation transaction，拆分 owner、Surface、EGL、
  GL 错误码，并将冷启动 VNC、立即重连和其他协议回归加入验收。
- 设备日志 blocker：已定位 DevEco `hdc`，但 `hdc list targets` 和 `hdc shell` 均返回
  `Connect server failed`；尚未取得本次现场的 `hilog`，不把静态结论写成真机日志证据。
- 当前调查分支：`codex/rustdesk-complete-repair@08185477f`，工作树含大量其他任务改动；本次只改了
  计划文档和本状态记录，未修改业务代码、未暂存、未提交。
- 本次文档收口验证：`default@OhosTestCompileArkTS` 成功（exit 0）；signed
  `assembleHap` 成功（`BUILD SUCCESSFUL`，exit 0）；Light 开源合规通过；`git diff --check`
  通过。构建警告为当前 checkout 已有 ArkTS/依赖弃用与严格类型警告，没有新的构建错误。

## Current VNC 证书、设置与中继 V3 计划（2026-08-01，仅文档）

- 只读审计和实体计划已落盘：
  `docs/superpowers/plans/2026-08-01-vnc-certificate-settings-relay-complete-repair-plan-v3.md`。
  当前任务没有修改 ArkTS、native、测试、配置、设备数据或 Git 历史。
- VNC TLS 首次/变化确认将仿照现有 RDP 证书 `bindSheet`，具备 probe、首次信任、
  已信任匹配、证书变化、错误五态，以及证书详情、取消、重试、仅本次继续、
  信任/更新并信任；真实连接仍由 native SHA-256 pin 做最终 fail-closed 校验。
- Trust 绑定真实 TLS endpoint：direct 归 VNC host，Repeater 归 VNC Gateway；
  云端 trust 仍只是候选，新设备必须实时 probe 后在本机重新确认。
- 主设置页“数据安全”将像 SSH 指纹、RDP 证书一样新增独立
  “VNC TLS 证书信任”管理；VNC 协议分组仅打开同一 manager，不能保留第二套
  可变 trust 列表或 owner。
- 同一后续独立任务还需修复 default transport/default Gateway、Gateway target
  owner、repeater TLS 归属、host/gateway 交叉校验、重复滚轮权威、无效
  `jpegQuality/custom/tight` 语义和 TCP 测试误判。
- 实施前置：另立唯一 VNC 任务分支；先写失败策略测试，再做结构化 TLS probe、
  Sheet/trust v2、设置一致性、mode12 深度检查，最后完成真实 direct TLS、
  UltraVNC Repeater、API 23、AGC 双设备和其他协议回归。当前没有实施授权。
- 本次文档门禁：精确 `default@OhosTestCompileArkTS` 和 signed `assembleHap`
  均 `BUILD SUCCESSFUL`（non-daemon、exit 0）；Light 开源合规 PASS；
  `git diff --check` PASS。校验时工作树同时存在另一任务的 cloud-sync/版本升级
  修改，本计划未修改、暂存或回退那些文件。
