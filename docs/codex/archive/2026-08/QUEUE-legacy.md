...
- **P0：VNC 渲染器初始化回归调查/修复计划已并入 V3（仅文档）**：当前 checkout 的
  `NapiInitRenderer()` 在 `GLRenderer::Init()` 前要求 active owner，但
  `RemoteDesktop.doConnect()` 的 renderer 初始化早于 `NapiConnect()` owner 创建，导致 VNC 冷启动
  `rc=-1`。后续实现必须采用 reservation → owner bind → activation transaction，不能伪造/复用旧 owner；
  并拆分 owner、Surface、EGL、GL 错误码。设备 `hdc` 当前 `Connect server failed`，真机日志和 API 23
  首帧/重连验收待设备恢复后补抓。

- VNC 证书弹窗、设置一致性与中继可用性 V3 计划已落盘：
  `docs/superpowers/plans/2026-08-01-vnc-certificate-settings-relay-complete-repair-plan-v3.md`。
  后续独立任务将用 RDP 风格五态 Sheet 完成 VNC TLS 首次/变化确认，把 direct
  trust 绑定 host、Repeater trust 绑定 Gateway，并在“数据安全”新增独立
  “VNC TLS 证书信任”管理；VNC 分组只能跳转同一 manager。计划同时覆盖默认
  transport/Gateway、target/TLS owner、设置死字段、共享滚轮权威、跨记录校验和
  mode12 分层健康检查。当前仅有计划文档，未实施代码；真实 TLS/Repeater、AGC
  双设备和 API 23 矩阵通过前保持 NO-GO。

- VNC RustDesk 操控对标与 ZRLE 客户端热路径已在本地 `main@4c22e1f4c` 完成：
  direct tile 解码约 `10-25ms`、GPU 提交约 `6ms`，双指/实体滚轮改为 RustDesk
  原始增量、最小步进、30ms/0.97 惯性和 burst 加速；共享个性化滚轮方向、外接
  键鼠自动模式 2、手动收键盘 stale 状态均已修复。native `182/182` 与两项
  Hvigor 门禁通过，最终 HAP 已安装；有线日志已确认实体轮按 4/12 tick 输出。
  仍需用“极速 + 色深自动”重连并记录 `effectiveDepth=8`、FPS、滚轮正反向、
  键鼠自动切换和键盘-控制台链路。
  标准 macOS Retina VNC 的剩余帧耗在服务端整帧抓取/压缩/读取和 zlib inflate；
  若产品目标硬性要求稳定 30fps，应另立高性能视频传输（如 VP9/AV1/H.264/H.265）
  或远端降分辨率方案，不要继续把 GL 微调当作主路径，也不要回退到实测更慢的 RAW。

- 旧的 VNC `0.79-0.81s`/frame 与 native `178/178` checkpoint 已被上方最新
  `182/182`、direct decode 和 RAW A/B 结论取代；不要恢复“继续试 RAW”的旧路径。
