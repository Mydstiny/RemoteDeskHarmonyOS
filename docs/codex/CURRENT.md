...
- Latest code checkpoint: `57f82b5` (`fix(ssh): stabilize SFTP root bind sheet opening`), following `23043aa` (`fix(ssh): make SFTP root sheet open synchronously`), `9abb4ce` (`fix(ssh): unblock SFTP bindSheet reopen`), `05db28c` (`fix(ssh): serialize SFTP bindSheet dismiss drivers`), `3d3c246` (`fix(ssh): fence SFTP root sheet dismissal`), `7117e21` (`fix(ssh): serialize SFTP Sheet dismissals`), `1587614` (`fix(ssh): gate nested SFTP picker on root appearance`), `52c9326` (`fix(ssh): queue nested SFTP picker mount`), `7fd01c6` (`fix(ssh): wait for nested SFTP sheet exit`), `c34c5e1d3` (`docs(codex): record SFTP sheet race gate`), `0c3be391b` (`fix(ssh): fence nested SFTP picker reopens`), `7892f0332` (`fix(ssh): keep Pad SFTP root sheet wide`), `af837c470` (`fix(ssh): harden forwarding runtime limits`), `ae05bc170` (`fix(ssh): guard failed forwarding profile replacement`), `30d2281dd` (`docs(codex): record fresh ssh sftp gate`), `cc68965` (`fix(ssh): make Pad SFTP root sheet explicit`), `f72b5fb` (`feat(ssh): expose keyboard-interactive auth prompts`), `caefd47` (`fix(ssh): keep Pad SFTP in wide root sheet`), `5e4b563` (`fix(ssh): bind wide SFTP layout to Pad devices`), `f6ef9559a` (`fix(ssh): size SFTP bind sheets`) and `9eb54c7f2` (`feat(ssh): refine SFTP endpoint workspace`); keyed surface recovery, forwarding lifecycle, adapter reactor binding, and the generation-guarded NAPI/ArkTS bridge are committed.
...
- SFTP bindSheet race follow-up: root and nested sheets fence callbacks by mount token, require the root content's native `onAppear` before mounting the child picker, re-run queued requests after that appearance callback, cancel requests before native mount, hold reopen requests until native `onDisappear`, and reject stale callbacks during rapid reopen, route back, and page teardown. The root open now commits `showSftpSheet=true` in the same UI turn; phone roots use a bounded explicit height with a full-height scroll viewport, and an idle visible-state check clears a stale old-dismiss latch before a new open. Its internal surface-readiness gate is no longer rendered `@State`, so the entrance animation cannot be invalidated by a page rebuild or a delayed open timer. Each mounted root/child Sheet has a one-shot `nativeDismissInFlight` driver; binding/programmatic close and interactive close both finish the native transition, while stale callbacks dismiss only their old native instance and never mutate the current mount. Bound visibility/state cleanup waits for `onDisappear`, and parent close still waits for a child already exiting. Root dismissal gates interactive callbacks on the live root surface and transfer state, while an explicit close bypasses those transient guards; the old fixed 500ms dismissal heuristic is removed. The added policy tests, `default@OhosTestCompileArkTS`, `assembleHap`, and `git diff --check` passed on 2026-08-07. HAP was installed to API 23 emulator `127.0.0.1:5555`; SFTP popup acceptance remains pending because the available SSH endpoint failed KEX preflight. No device screenshot is claimed.
...
- Pad SFTP root-sheet follow-up `cc68965` keeps the complete wide workspace in the Pad `bindSheet`, binds the root sheet geometry directly to `isPadDevice`, resets a newly opened right pane to chooser-only, and moves the close affordance below the sheet inset. The current `ae05bc170` gates remain green after the retry above; no device screenshot is claimed.
- Auth prompt checkpoint `f72b5fb`, Pad SFTP root-sheet checkpoint `cc68965`, and forwarding runtime checkpoint `af837c470` are code-only and await the existing SSH reviewer task's incremental review; status remains `REVIEW_REQUIRED` because the worktree still contains preserved mixed-protocol changes.
