...
- Latest code checkpoint: `c62a050` (`fix(ssh): stabilize SFTP picker lifecycle`), following `f1df700` (`fix(ssh): isolate SFTP root bind sheet host`), `57f82b5` (`fix(ssh): stabilize SFTP root bind sheet opening`), `23043aa` (`fix(ssh): make SFTP root sheet open synchronously`), `9abb4ce` (`fix(ssh): unblock SFTP bindSheet reopen`), `05db28c` (`fix(ssh): serialize SFTP bindSheet dismiss drivers`), `3d3c246` (`fix(ssh): fence SFTP root sheet dismissal`), `7117e21` (`fix(ssh): serialize SFTP Sheet dismissals`), `1587614` (`fix(ssh): gate nested SFTP picker on root appearance`), `52c9326` (`fix(ssh): queue nested SFTP picker mount`), `7fd01c6` (`fix(ssh): wait for nested SFTP sheet exit`), `c34c5e1d3` (`docs(codex): record SFTP sheet race gate`), `0c3be391b` (`fix(ssh): fence nested SFTP picker reopens`), `7892f0332` (`fix(ssh): keep Pad SFTP root sheet wide`), `af837c470` (`fix(ssh): harden forwarding runtime limits`), `ae05bc170` (`fix(ssh): guard failed forwarding profile replacement`), `30d2281dd` (`docs(codex): record fresh ssh sftp gate`), `cc68965` (`fix(ssh): make Pad SFTP root sheet explicit`), `f72b5fb` (`feat(ssh): expose keyboard-interactive auth prompts`), `caefd47` (`fix(ssh): keep Pad SFTP in wide root sheet`), `5e4b563` (`fix(ssh): bind wide SFTP layout to Pad devices`), `f6ef9559a` (`fix(ssh): size SFTP bind sheets`) and `9eb54c7f2` (`feat(ssh): refine SFTP endpoint workspace`); keyed surface recovery, forwarding lifecycle, adapter reactor binding, and the generation-guarded NAPI/ArkTS bridge are committed.
- Phase: Alacritty core and xterm-safe visible fallback; lifecycle, VT/Unicode/resize/TUI/large-output parity, per-host output isolation, strict ACK-gated canvas presentation, SFTP Pad/phone workspace and background-session handoff, host-switch/surface refresh recovery, guarded surface fallback, and native forwarding contract.
...
- Mobile SFTP keeps the current SSH host on the left and exposes exactly two right-endpoint choices: local files or an SSH host. The initial right pane is chooser-only; selection is committed only after a button/host choice. The root SFTP bindSheet and the host picker now use separate sibling bindSheet hosts, so opening, closing, cancelling, selecting and changing hosts cannot reconcile a hidden picker into the root surface. Current-host selection reuses the foreground session. Each newly opened SFTP surface resets the right endpoint to chooser-only. Phone stays on the bottom bindSheet; Pad uses explicit 96% SFTP width and 88% host-picker width, with the header close affordance below the sheet inset. The wide remote list uses weighted flex height instead of a competing 100% constraint, and path jump plus rename remain on one row. A shared 320 ms exit guard delays the host-list add-host Sheet during cross-route back navigation, and the inactive host-list FAB is hidden during route teardown.
- SSH forwarding validates local/remote/dynamic profiles, binding policy, bounded connections, byte/expiry limits, generations and start/listen/fail/stop transitions. `SshAdapter` owns the manager and reactor reset, cleans stale runtimes, preserves TCP half-close relay buffers, and fences late releases; NAPI/ArkTS exposes the guarded lifecycle API, while real libssh2 socket/channel forwarding remains open.
...
- The current SSH canvas follow-up keeps the terminal shell at the full post-header height, reserves the virtual-key bar height in xterm, anchors the bar to the complete shell, adds a 10px xterm bottom safety inset plus a 10px viewport offset, and refreshes keyboard plus keyboard/mouse presence at the SSH connection boundary using the RDP-aligned device snapshot.
...
- RDP Gateway-aware certificate preflight keeps Gateway/target certificates and route identity separate; `CONTINUE_ONCE` carries temporary dual pins without changing persisted trust records. Vendor HTTPS, Azure Bastion and unknown Gateway routes fail closed. Restricted Admin + Microsoft RD Gateway is explicitly rejected because the target-only NTLM hash is not a Gateway password; real Gateway interoperability remains unverified.
...
- Host native tests: `294 passed, 16 failed, 310 total`; failures are existing
...
- Current canvas follow-up: `default@OhosTestCompileArkTS` passed and `assembleHap` returned `BUILD SUCCESSFUL` after the shell/bar/keyboard changes on 2026-08-07; `git diff --check` passed.
- SSH xterm prewarm follow-up: the hidden host-list WebView now records its lifecycle and performs one API 23 proxy-activation reload only when bridge registration misses the first document. Both Hvigor gates and `git diff --check` passed; HDC `127.0.0.1:5555` reached `prewarm ready` in 523 ms, then visible SSH reached `page begin` about 108 ms after the first connection completion and about 102 ms on repeat entry. No duplicate reload, crash, `SIGABRT`, `Device lost` or `SurfaceFlush` was observed; second-host and virtual-keybar acceptance remain open.
...
- Mobile SFTP UI polish follow-up: right endpoint state resets on each new surface, the SFTP close affordance is moved down for the Pad/phone sheet inset, the empty host-list hint no longer renders a second `+`, and the inactive host-list FAB is hidden during route teardown. Checkpoint `c62a050` keeps root and picker content mounted until their own native exit, uses bounded close recovery, cancels stale picker choices, lets `更换 host` reopen the picker while an old peer read is pending, reuses the current SSH session, and gives the Pad remote list a flex viewport. `default@OhosTestCompileArkTS`, `assembleHap` and `git diff --check` passed on 2026-08-07; HDC currently reports no online target, so this iteration has no new device smoke claim.
- SFTP bindSheet race follow-up: root and host-picker Sheets are separate sibling hosts, each callback is fenced by mount token, picker opening waits for root onAppear, queued requests are cancellable, and selection/cancel/close wait for the picker onDisappear or bounded recovery. The root close path keeps content through native exit and stale callbacks cannot mutate a new mount. The initial right pane remains chooser-only; current-host SFTP reuses the active session. The previous API 23 Pad smoke covered root open/close/reopen and picker open/close, while this checkpoint still needs a new device run for repeat host switching.
...
- Keyboard-interactive prompt UI follow-up: `default@OhosTestCompileArkTS` passed and `assembleHap` returned `BUILD SUCCESSFUL` after adding multi-prompt input, system-dismiss cancellation and stale-snapshot suppression on 2026-08-07; the host native suite now reports `291 passed, 16 pre-existing VNC TLS fixture startup failures, 307 total`. No real MFA endpoint or device screenshot is claimed.
- Fresh full-worktree gate rerun on 2026-08-07 passed both required Hvigor commands and `git diff --check`; the earlier first `assembleHap` attempt exposed an existing RDP `-Werror` unused certificate helper in the preserved mixed-worktree changes, then passed after a `[[maybe_unused]]` annotation with no runtime behavior change.
- Current `af837c470` rerun: `default@OhosTestCompileArkTS` passed; the first `assembleHap` packaging attempt hit a transient BundleTool `Bad size for entry resources/rawfile/totp-brands/catalog/microsoftteams 2.svg` error, and the immediate retry returned `BUILD SUCCESSFUL`. Source and intermediate resource sizes matched; no product file was changed.
- Direct real-FreeRDP OHOS objects (`freerdp_adapter.cpp` and `rdp_file_clipboard_bridge.cpp`) compile successfully; the fresh HAP also passes the production assemble gate above.
- RDP Gateway policy checkpoint: direct/transparent routes retain their existing path;
...
- HDC reproduction: SSH TCP/KEX/public-key auth/PTY/shell all completed; the
  historical crash was `DrawSnapshot -> OH_Drawing_SurfaceFlush -> Device lost -> SIGABRT`
  on PID 18229 at 2026-08-06 17:36:30. The latest prewarm/xterm runs on
  `127.0.0.1:5555` reached prewarm `ready` plus visible `page ready`/`report ready`
  on first and repeat entry, and showed no `SIGABRT`, `Device lost`, `SurfaceFlush`
  or renderer crash. The remaining first-entry delay is in native async completion,
  not the visible xterm mount.
...
- Native forwarding manager tests are included in the host run and pass; the
  ...
  compilation passed. The current run is `294 passed, 16 failed, 310 total`,
  with all failures remaining in the pre-existing VNC TLS fixture startups.
...
- Pad SFTP root-sheet follow-up `cc68965` plus `f1df700` keeps the complete wide workspace in the Pad `bindSheet`, binds the root sheet geometry directly to `isPadDevice`, isolates its host, resets a newly opened right pane to chooser-only, and moves the close affordance below the sheet inset. The latest signed HAP was installed and the real Pad open/close/reopen/picker smoke passed.
- Auth prompt checkpoint `f72b5fb`, Pad SFTP root-sheet checkpoints `cc68965`/`f1df700`, and forwarding runtime checkpoint `af837c470` are code-only and await the existing SSH reviewer task's incremental review; status remains `REVIEW_REQUIRED` because the worktree still contains preserved mixed-protocol changes.
...
- RDP Gateway evidence is code/test-only: no real Microsoft RD Gateway, certificate-rotation,
  transport-method, or Restricted Admin Gateway endpoint evidence; `CONTINUE_ONCE` persistence isolation is covered by policy test.
- HDC target `5KLBB25928203528` remains offline for codec evidence; the current
  `127.0.0.1:5555` target is online and the signed HAP is installed. No
  three-codec frame log or physical-device screenshot is claimed yet.
- Real Pad root-sheet interaction is verified; mobile SFTP transfer, background restart and real endpoint interoperability remain unverified.
## Next
1. User-accept second-host switching plus virtual-keyboard/function-bar re-entry on the installed xterm path; keep the Native Drawing backend disabled until a safe GPU backend exists.
