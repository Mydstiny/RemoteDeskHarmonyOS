...
- Latest code checkpoint: `f1df700` (`fix(ssh): isolate SFTP root bind sheet host`), following `57f82b5` (`fix(ssh): stabilize SFTP root bind sheet opening`), `23043aa` (`fix(ssh): make SFTP root sheet open synchronously`), `9abb4ce` (`fix(ssh): unblock SFTP bindSheet reopen`), `05db28c` (`fix(ssh): serialize SFTP bindSheet dismiss drivers`), `3d3c246` (`fix(ssh): fence SFTP root sheet dismissal`), `7117e21` (`fix(ssh): serialize SFTP Sheet dismissals`), `1587614` (`fix(ssh): gate nested SFTP picker on root appearance`), `52c9326` (`fix(ssh): queue nested SFTP picker mount`), `7fd01c6` (`fix(ssh): wait for nested SFTP sheet exit`), `c34c5e1d3` (`docs(codex): record SFTP sheet race gate`), `0c3be391b` (`fix(ssh): fence nested SFTP picker reopens`), `7892f0332` (`fix(ssh): keep Pad SFTP root sheet wide`), `af837c470` (`fix(ssh): harden forwarding runtime limits`), `ae05bc170` (`fix(ssh): guard failed forwarding profile replacement`), `30d2281dd` (`docs(codex): record fresh ssh sftp gate`), `cc68965` (`fix(ssh): make Pad SFTP root sheet explicit`), `f72b5fb` (`feat(ssh): expose keyboard-interactive auth prompts`), `caefd47` (`fix(ssh): keep Pad SFTP in wide root sheet`), `5e4b563` (`fix(ssh): bind wide SFTP layout to Pad devices`), `f6ef9559a` (`fix(ssh): size SFTP bind sheets`) and `9eb54c7f2` (`feat(ssh): refine SFTP endpoint workspace`); keyed surface recovery, forwarding lifecycle, adapter reactor binding, and the generation-guarded NAPI/ArkTS bridge are committed.
...
- Mobile SFTP keeps the current SSH host on the left and exposes exactly two right-endpoint choices: local files or an SSH host. The initial right pane is chooser-only; selection is committed only after a button/host choice, and host selection uses a nested bindSheet inside the existing SFTP surface. Each newly opened SFTP surface resets the right endpoint to chooser-only. Phone stays on the bottom bindSheet; Pad uses explicit 96% SFTP width and 88% host-picker width, with the header close affordance lowered below the sheet edge inset. The root SFTP bindSheet now has an independent host node, so passphrase/auth/tab sheets cannot dismiss it during entrance; its first directory read waits for root onAppear. A shared 320 ms exit guard delays the host-list add-host Sheet during cross-route back navigation, and the inactive host-list FAB is hidden during route teardown so it cannot duplicate the SSH-page affordance.
...
- SFTP bindSheet race follow-up: root and nested sheets fence callbacks by mount token, require the root content's native `onAppear` before mounting the child picker, re-run queued requests after that appearance callback, cancel requests before native mount, hold reopen requests until native `onDisappear`, and reject stale callbacks during rapid reopen, route back, and page teardown. The root SFTP host is now independent from the outer auth/tab Sheet chain, and the first directory read waits for root onAppear so entrance rebuilds cannot dismiss the root. Each mounted root/child Sheet has a one-shot `nativeDismissInFlight` driver; binding/programmatic close and interactive close both finish the native transition, while stale callbacks dismiss only their old native instance and never mutate the current mount. The added policy tests, both required Hvigor gates, `git diff --check`, and API 23 Pad smoke passed on 2026-08-07: root opened with 24 entries, closed, reopened, and nested SSH-host picker opened/closed while the parent stayed visible. The initial right pane remained chooser-only.
