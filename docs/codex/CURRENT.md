...
- Latest code checkpoint: `23043aa` (`fix(ssh): make SFTP root sheet open synchronously`), following `9abb4ce` (`fix(ssh): unblock SFTP bindSheet reopen`), `05db28c` (`fix(ssh): serialize SFTP bindSheet dismiss drivers`), `3d3c246` (`fix(ssh): fence SFTP root sheet dismissal`), `7117e21` (`fix(ssh): serialize SFTP Sheet dismissals`), `1587614` (`fix(ssh): gate nested SFTP picker on root appearance`), `52c9326` (`fix(ssh): queue nested SFTP picker mount`), `7fd01c6` (`fix(ssh): wait for nested SFTP sheet exit`), `c34c5e1d3` (`docs(codex): record SFTP sheet race gate`), `0c3be391b` (`fix(ssh): fence nested SFTP picker reopens`), `7892f0332` (`fix(ssh): keep Pad SFTP root sheet wide`), `af837c470` (`fix(ssh): harden forwarding runtime limits`), `ae05bc170` (`fix(ssh): guard failed forwarding profile replacement`), `30d2281dd` (`docs(codex): record fresh ssh sftp gate`), `cc68965` (`fix(ssh): make Pad SFTP root sheet explicit`), `f72b5fb` (`feat(ssh): expose keyboard-interactive auth prompts`), `caefd47` (`fix(ssh): keep Pad SFTP in wide root sheet`), `5e4b563` (`fix(ssh): bind wide SFTP layout to Pad devices`), `f6ef9559a` (`fix(ssh): size SFTP bind sheets`) and `9eb54c7f2` (`feat(ssh): refine SFTP endpoint workspace`); keyed surface recovery, forwarding lifecycle, adapter reactor binding, and the generation-guarded NAPI/ArkTS bridge are committed.
