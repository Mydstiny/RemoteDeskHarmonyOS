...
- SFTP bindSheet race follow-up: root and nested sheets now fence callbacks by mount token, hold reopen requests until native `onDisappear`, cancel queued child-picker reopens when the parent closes, and reject stale picker callbacks during rapid reopen, route back, and page teardown. Parent close also recognizes a picker already in exit animation and waits for the child's native `onDisappear`; the old fixed 360ms parent-close fallback is removed. The event-bridge cast newline blocker was fixed; the final `default@OhosTestCompileArkTS`, `assembleHap`, fresh native rebuild/test, and `git diff --check` passed on 2026-08-07. No device screenshot is claimed.
