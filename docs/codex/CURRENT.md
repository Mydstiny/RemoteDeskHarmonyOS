...
- SFTP bindSheet race follow-up: root and nested sheets now fence callbacks by mount token, hold reopen requests until native `onDisappear`, cancel queued child-picker reopens when the parent closes, and reject stale picker callbacks during rapid reopen, route back, and page teardown. The event-bridge cast newline blocker was fixed; the final `default@OhosTestCompileArkTS`, `assembleHap`, and `git diff --check` passed on 2026-08-07. No device screenshot is claimed.
