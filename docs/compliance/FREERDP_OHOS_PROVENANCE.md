...
the five ordered patches under `patches/freerdp-ohos/`. Applying the series
must produce Git tree `24a880d801892e3d6f1b8c78534e51eaeca8b0d8`. The first four
patches reconstruct the source tree formerly represented only by unpublished
local commit `4d645c86e1fdcc6159b2b3a4c4f652e46985f8ba`; the fifth records the
reviewed IPv6 gateway identity and resolver-lifecycle hardening. Public base
reachability was verified against the configured repository on 2026-08-30.
...
sockets are closed and the winning socket is restored to blocking mode. The
test seam proves that timeout and cancellation return without waiting for a
blocked resolver, while its late result is released exactly once. Gateway
tests bind scoped IPv6 transport routing independently from TLS, HTTP Host and
SPN identity.
...
from different temporary roots. Their complete prebuilt trees were
