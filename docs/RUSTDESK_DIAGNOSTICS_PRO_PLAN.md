...
- 版本：v0.4（同步 2026-07-21 性能监视可用性与停靠修复）
- 复核日期：2026-07-21
...
本轮针对已实现版本的真实使用问题执行以下闭环；代码已完成，仍需真机交互验收：

1. Pro 开关不再由基础开关禁用。用户直接开启 Pro 时会同步开启基础监视；关闭基础监视仅使 Pro 暂不生效，保留其本地偏好。
2. 诊断快照拆分为会话、协议快照、视频回调、接收采样、真实呈现采样等独立可用性状态。回调帧率、分辨率和编码信息不再因 FFI 延迟快照暂缺而被整块隐藏。
3. 接收 FPS 和解码提交 FPS 使用完成的回调时间窗；真实送显 FPS 使用 GL swap 窗口的帧数除以窗口时长，禁止将窗口帧数直接当作 FPS。
4. HUD 明确显示“等待视频流”“采样中”“已停滞”和“协议采样不可用”，避免将零、NaN 和尚未完成的采样混为同一不可用状态。
5. 性能监视改为独立侧边停靠控件：折叠时仅保留 28vp 贴边标签，点击展开，拖动后左/右吸附并持久化侧别与纵向比例。默认与修饰键胶囊相反侧，同侧时避让其展开面板，并约束顶部栏、键盘和文件拖放提示区。

本轮自动化验证：native `rdp_native_tests` 114/114、`default@OhosTestCompileArkTS`、非 daemon 生产 `assembleHap`、`git diff --check`。真实 HarmonyOS 设备仍需验证首帧后的 FPS、折叠/拖动、横竖屏、键盘显示、拖放提示和重连后的停靠位置。
