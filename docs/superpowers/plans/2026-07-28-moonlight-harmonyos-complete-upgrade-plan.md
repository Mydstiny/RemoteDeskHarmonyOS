...
U1-06～U1-12 的完成含义是“本地 UI/路由/设置合同已落地”，不是产品已开放 Moonlight。下一实现边界为
...
当前唯一直接实现边界是 S1-01/S1-02：只把 UI shell 接到唯一 remote-session registry/coordinator；不得把
cache 或请求态伪装成在线/已刷新，也不接真实 Host Control/runtime/cloud。
