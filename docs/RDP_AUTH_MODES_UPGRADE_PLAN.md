...
| Restricted Admin | 目标端开启 `DisableRestrictedAdmin=0`，并满足管理员、NTLM、策略等条件 | 用户提供的账户 NTLM Hash | 条件可实现 | 增加第三模式，但不能承诺“只输入用户名” |
...
- 支持空密码本地账户的显式保存和连接。
- 支持 Restricted Admin 的显式模式、NTLM Hash 输入和错误提示；不把空密码账户 Hash 伪装成通用 Restricted Admin 凭据。
...
- 历史 `empty_password_hash` 值只做兼容读取并归一化为 `ntlm_hash`；不会再生成 `31D6...` 常量，用户必须重新输入真实账户 Hash。
...
- 空密码账户应使用独立的 `blank_password` 模式；Restricted Admin 不从“空密码”推导 Hash。
...
- 显示用户名输入框和可选域/用户名提示。
- 显示“账户 NTLM Hash（必填）”；不提供“空密码 Hash”选项。
...
4. `restricted_admin` 从本机 `localextensions` 读取并解密用户提供的 Hash；若 Hash 缺失、DataCrypto 锁定或绑定不匹配，返回本地错误，不调用 native。
...
| Restricted Admin 同步到新设备 | 同步模式和 Hash 来源，不同步 Hash；首次连接提示重新输入 Hash |
...
- 确认真实 FreeRDP `/restricted-admin` 与 `/pth` 的 Hash 路径；空密码账户只按独立 `blank_password` 模式验收。
...
### 13.2 代码审计修复记录（2026-07-22）

- 按仓库内 FreeRDP 上游 `/restricted-admin` 与 `/pth` 实现收紧 Native 契约：普通密码/空密码模式拒绝携带 Hash，Restricted Admin 只接受 32 位账户 NTLM Hash；遗留 `empty_password_hash` 只归一化为“需要重新输入 Hash”，不再生成空密码常量 Hash。
- 统一 ArkTS 与 native 的有效用户名/域规范化，并将本机密文绑定升级为目标地址、端口、主机名、有效用户名、有效域和身份模式；绑定不匹配时 fail closed。
- 现代 FAB/经典编辑入口只显示真实 Hash 输入；编辑保存失败时清理已预写入的本机 Hash，避免出现孤立或与主机行不一致的密文。
- FreeRDP Hash 在配置实例后尽快清理 ArkTS/`ConnectionConfig` 临时副本，实例销毁前覆写 settings 中的 Hash；认证错误码不再被统一折叠为无操作指引的连接失败。
- 自动验证重新通过：native 124/124、`default@OhosTestCompileArkTS`、非 daemon 生产 `assembleHap`、`git diff --check` 和 Light 合规门；Windows/HarmonyOS 首帧、重连、策略矩阵仍交由用户实机验收。
...
- Hash 校验、遗留空密码 Hash 来源拒绝、模式冲突和诊断码映射。
...
6. 遗留“空密码 Hash”配置必须被拒绝并要求重新输入真实 Hash；空密码账户走 `blank_password` 矩阵。
...
- Restricted Admin 至少完成“用户输入 NTLM Hash”真实 HAP 连接验收；空密码账户必须单独完成 `blank_password` 实机验收。
